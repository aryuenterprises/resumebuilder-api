...
const createFinalizeResume = async (req, res) => {
    try {
        const { contactId, languages, certificationsAndLicenses, hobbiesAndInterests, awardsAndHonors, websitesAndSocialMedia, references, customSection, } = req.body;
...
//   }
// };
const updateFinalizeResume = async (req, res) => {
    try {
        const { id, contactId } = req.query;
        const { skillsData, templateId } = req.body;
        if (!contactId) {
        ...
        if (!existingFinalize) {
            ...
            if (templateId) {
                ...
                for (const Model of modelsToUpdate) {
                    ...
                    await Model.updateMany({ contactId }, { templateId: String(templateId) });
                }
            }
            return res.status(201).json({
                message: "Finalize created successfully",
        ...
        if (skillsData)
            existingFinalize.skillsData = skillsData;
        if (templateId)
            existingFinalize.templateId = String(templateId);
...
const getFinalizeResumeById = async (req, res) => {
    try {
        const { id } = req.params;
        const resume = await FinalizeResume.find({ contactId: id }).populate("contactId");
...
const getFinalizeResume = async (req, res) => {
    try {
