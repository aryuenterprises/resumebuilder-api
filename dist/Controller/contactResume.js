...
//   }
// };
const allContactResume = async (req, res) => {
    const { id } = req.params;
    try {
        // if (!id) {
        //   return res.status(400).json({ message: "User ID is required" });
...
const getAllContactResume = async (req, res) => {
    try {
        const resumes = await ContactResume.find();
        res.json(resumes);
    }
...
const createContactResume = async (req, res) => {
    try {
        const photoFile = req.files?.find((file) => file.fieldname === "photo");
        const photo = photoFile ? photoFile.filename : null;
        const { userId, firstName, lastName, email, jobTitle, keywords, tones, phone, country, city, address, postCode, linkedIn, portfolio, } = req.body;
...
// GET contact resumes
const getContactResume = async (req, res) => {
    const { id } = req.params;
    const { resumeId } = req.query;
    try {
        if (resumeId) {
...
const updateResume = async (req, res) => {
    try {
        ...
        const { firstName, lastName, email, jobTitle, keywords, tones, phone, country, city, address, postCode, linkedIn, portfolio, templateId, } = req.body;
        let existingResume;
        if (id) {
...
export { createContactResume, updateResume, getContactResume, getAllContactResume, allContactResume, };
