    ...
    try {
        ...
        const result = contacts.map((contact) => {
            ...
            const finalize = finalizeResumes
                .filter((finalizeResumes) => finalizeResumes.contactId?.toString() === contactIdStr)
    ...
    try {
        ...
        const result = contacts.map((contact) => {
            ...
            return {
                ...
                finalize: finalize,
                planSubscriptions: {
                    planId: planSubscriptions.find((ps) => ps?.userId?.toString() === contact.userId.toString())?.planId || null,
                    status: planSubscriptions.find((ps) => ps?.userId?.toString() === contact.userId.toString())?.status || "none",
                    amount: planSubscriptions.find((ps) => ps?.userId?.toString() === contact.userId.toString())?.amount || 0,
                    paymentDetails: planSubscriptions.find((ps) => ps?.userId?.toString() === contact.userId.toString())?.paymentDetails || null,
                },
            };
        });
        res.status(200).json({
...
export { getAllContacts, createExperience, updateExperience, getExperienceById, getUserContacts, };
