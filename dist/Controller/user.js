...
const getAllUsers = async (req, res) => {
    try {
        ...
        const planSubscriptions = await Promise.all(users.map(async (user) => {
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const settingDetails = await setting.findOne().lean();
        const fromName = settingDetails ? settingDetails.fromName : 'ResumeMint';
        await sendEmail(email, `Your ${fromName} Password Reset OTP`, "otpEmail.html", {
            firstName: user.firstName,
            fromName,
            otp,
        });
    ...
    try {
        ...
        if (type === "download") {
            const payments = await Payment.find({ userId: userId })
                ...
                .lean();
            const paymentsToUpdate = payments.filter((payment) => payment.planId?.name !== "Lifetime Full Access Option");
...
const downloadResume = async (req, res) => {
    try {
        ...
        /* --------------------------------------------
                     Update Resume
        --------------------------------------------- */
        const updatedResume = await ContactResume.findByIdAndUpdate(contactId, {
            resume: resumeFile.filename,
            resumeStatus: message,
        }, { new: true });
        console.log("Updated Resume:", updatedResume);
    ...
    try {
        ...
        if (user.isVerified === false) {
            ...
            return res.status(403).json({
                message: "User verification is pending. We have resent the verification link to your registered email address.",
            });
        }
        // if (user.isVerified === false) {
        ...
        const token = jwt.sign({ id: user._id, email: user.email }, process.env.JWT_SECRET || "your_secret_key", { expiresIn: "1h" });
        ...
        res.status(200).json({
            ...
            user: {
                ...
                planName: payments?.planId?.name || "",
                planPrice: payments?.planId?.price || "",
...
const editUser = async (req, res) => {
    ...
    const user = await User.findByIdAndUpdate(req.params.id, { firstName, lastName, email, phone, city, state, country, status }, { new: true });
    if (!user)
        return res.status(404).json({ message: "User not found" });
    res.json(user);
};
...
export { downloadResume, dashboard, getAllUsers, getUserById, addUser, editUser, deleteUser, loginUser, forgotPassword, verifyEmail, verifyOtpAndResetPassword, };
