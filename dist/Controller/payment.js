...
const fetchPaymentIntent = async (req, res) => {
    try {
        const { amount, metadata, userId, templateId, planId, currencyName, ...paymentDetails } = req.body;
        ...
        payment = await Payment.findOneAndUpdate({ userId }, {
            paymentId: paymentIntent.id,
            amount: paymentIntent.amount,
            planId,
            templateId,
...
const freePlan = async (req, res) => {
    ...
    try {
        ...
        //   { new: true, upsert: true }
        // );
        const payment = await Payment.findOneAndUpdate({ userId }, {
            planId,
            amount: 0,
            status: "succeeded",
            paymentId: uuidv4(),
...
const paymentUpdate = async (req, res) => {
    try {
        ...
        if (paymentIntent.status === "succeeded") {
            ...
            const updatedPayment = await Payment.findOneAndUpdate({ paymentId: paymentIntent.id }, {
                status: paymentIntent.status,
                currency: paymentIntent.currency,
                planId: planId,
                paymentDetails,
            }, { new: true });
            const updatedPaymentLog = await PaymentLog.findOneAndUpdate({ paymentId: paymentIntent.id }, { status: paymentIntent.status, planId: planId, paymentDetails }, { new: true });
            if (!updatedPayment) {
                return res.status(404).json({ error: "Payment record not found" });
            }
            return res.status(200).json({
